...
#2. Ricinus communis (HAnat)
#3. Marrubium vulgar (HAnat)

#Native:
#1. Malosma laurina Engl.(HAnat)
...
Heteromelesarbutifolia <- "Heteromeles arbutifolia"
HAnatObservations <- input[input$verbatimScientificName %in% Heteromelesarbutifolia, ]

write.csv(HAnatObservations, file="HAnatObservations")
...
HAnat_location <- HAnatObservations[, c("decimalLongitude", "decimalLatitude")]
...
HAnat_with_envi <- HAnat_location
for(envi_map_layer in envi_map_layers){
  envi_filename <- gsub("./EnvironmentalFactors/","",gsub(".tif","",envi_map_layer))
  # HAnat species
  HAnat <- as.data.frame(raster::extract(raster(envi_map_layer),HAnat_location))
  colnames(HAnat) <- envi_filename
  HAnat_with_envi <- cbind(HAnat_with_envi,HAnat)
}

HAnat_with_envi$pa <- 1

# write.csv(HAnat_with_envi, file="HAnat_with_envi.csv")
# write.csv(native_with_envi, file="native_with_envi.csv")

#Remove rows with missing data for observations points.
HAnat_with_envi <- na.omit(HAnat_with_envi)

# HAnat_with_envi$X <- NULL
...
lat_min <- 33
lat_max <- 35
#Get the larger of the two numbers: native or HAnat species observations.
obs_num <- 10*max(nrow(HAnat_with_envi))

#Create empty dataframe for random background points for HAnats.
#Make it much larger than the set of presence points.
background_HAnat_locations <- data.frame(matrix(nrow=obs_num,ncol=2))
colnames(background_HAnat_locations) <- c("decimalLongitude","decimalLatitude")
#Randomly generate longitude and latitude points.
background_HAnat_locations$decimalLongitude <- runif(obs_num,min=long_min,max=long_max)
background_HAnat_locations$decimalLatitude <- runif(obs_num,min=lat_min,max=lat_max)
...
HAnat_background <- background_HAnat_locations
for(envi_map_layer in envi_map_layers){
  envi_filename <- gsub("./EnvironmentalFactors/","",gsub(".tif","",envi_map_layer)) #Get short names for environmental layers.
  #Extract environmental values at HAnat species locations.
  HAnat_background_tmp <- as.data.frame(raster::extract(raster(envi_map_layer),background_HAnat_locations))
  colnames(HAnat_background_tmp) <- envi_filename
  HAnat_background <- cbind(HAnat_background,HAnat_background_tmp)
  #Extract environmental values at native species locations.
}

HAnat_background$pa <- 0



#Remove rows with missing data for observations points.
HAnat_background <- na.omit(HAnat_background)


#Run random forest models on your HAnat species data.
...
for(i in 1:model_num){
  pres_subset <- HAnat_with_envi[,colnames(HAnat_with_envi) %in% c(envi_filename,"pa")] #Remove latitude and longitude columns from presence point subset.
  pres_subset <- pres_subset[sample(nrow(pres_subset),sample_num),] #Subsample rows.
  abs_subset <- HAnat_background[,colnames(HAnat_background) %in% c(envi_filename,"pa")] #Remove latitude and longitude columns from presence point subset.
...
write.table(rf_importance_total_sum,"HAnatVarImp.txt",quote=FALSE,sep="\t",row.names = FALSE)
...
redo_HAnat_with_envi <- HAnat_location
for(envi_map_layer in envi_map_layers){
  envi_filename <- gsub("./EnvironmentalFactors/","",gsub(".tif","",envi_map_layer))
  # HAnat species
  HAnat <- as.data.frame(raster::extract(raster(envi_map_layer),HAnat_location))
  colnames(HAnat) <- envi_filename
  redo_HAnat_with_envi <- cbind(redo_HAnat_with_envi,HAnat)
  # native species
}

redo_HAnat_with_envi$pa <- 1
# 
# write.csv(redo_HAnat_with_envi, file="redo_HAnat_with_envi.csv")
# write.csv(redo_native_with_envi, file="redo_native_with_envi.csv")

#Remove rows with missing data for observations points.
redo_HAnat_with_envi <- na.omit(redo_HAnat_with_envi)
...
lat_max <- 35
#Get the larger of the two numbers: native or HAnat species observations.
obs_num <- 10*max(nrow(redo_HAnat_with_envi))

#Create empty dataframe for random background points for HAnats.
#Make it much larger than the set of presence points.
redo_background_HAnat_locations <- data.frame(matrix(nrow=obs_num,ncol=2))
colnames(redo_background_HAnat_locations) <- c("decimalLongitude","decimalLatitude")
#Randomly generate longitude and latitude points.
redo_background_HAnat_locations$decimalLongitude <- runif(obs_num,min=long_min,max=long_max)
redo_background_HAnat_locations$decimalLatitude <- runif(obs_num,min=lat_min,max=lat_max)
...
redo_HAnat_background <- redo_background_HAnat_locations
for(envi_map_layer in envi_map_layers){
  envi_filename <- gsub("./EnvironmentalFactors/","",gsub(".tif","",envi_map_layer))
  #Get short names for environmental layers.
  #Extract environmental values at HAnat species locations.
  redo_HAnat_background_tmp <- as.data.frame(raster::extract(raster(envi_map_layer),redo_background_HAnat_locations))
  colnames(redo_HAnat_background_tmp) <- envi_filename
  redo_HAnat_background <- cbind(redo_HAnat_background,redo_HAnat_background_tmp)
  #Extract environmental values at native species locations.
}

#Designate a presence/absence column for the pseudo-absence points.
redo_HAnat_background$pa <- 0



#Remove rows with missing data for observations points.
redo_HAnat_background <- na.omit(redo_HAnat_background)




HAnat__randomF_rf_importance_total <- data.frame()
HAnat__randomF_evaluation_total <- data.frame()

envi_filename <- gsub("./EnvironmentalFactors/","",gsub(".tif","",envi_map_layers))
for(i in 1:model_num){
  pres_subset <- redo_HAnat_with_envi[,colnames(redo_HAnat_with_envi) %in% c(envi_filename,"pa")] #Remove latitude and longitude columns from presence point subset.
  pres_subset <- pres_subset[sample(nrow(pres_subset),sample_num),] #Subsample rows.
  abs_subset <- redo_HAnat_background[,colnames(redo_HAnat_background) %in% c(envi_filename,"pa")] #Remove latitude and longitude columns from presence point subset.
  abs_subset <- abs_subset[sample(nrow(abs_subset),10*sample_num),] #Subsample rows.
  ...
  HAnat_rf_regress <- suppressWarnings(randomForest(envi_train[,colnames(envi_train) %in% envi_filename],envi_train$pa,stepFactor=1,plot=FALSE,doBest=TRUE))
  ...
  HAnat__randomF_rf_importance <- importance(HAnat_rf_regress)
  HAnat__randomF_rf_importance <- data.frame(names=row.names(HAnat__randomF_rf_importance),HAnat__randomF_rf_importance)
  HAnat__randomF_rf_importance_total <- rbind(HAnat__randomF_rf_importance_total,HAnat__randomF_rf_importance)
  
  HAnat__erf <- suppressWarnings(evaluate(testpres,testbackgr,HAnat_rf_regress))
  HAnat__randomF_rf_evaluation <-  data.frame(matrix(nrow=1,ncol=5))
  colnames(HAnat__randomF_rf_evaluation) <- c("AUC","cor","kappa","Q","TSS")
  HAnat__randomF_rf_evaluation$AUC <- HAnat__erf@auc
  HAnat__randomF_rf_evaluation$cor <- HAnat__erf@cor
  HAnat__randomF_rf_evaluation$kappa <- max(HAnat__erf@kappa)
  # Calculate Yule's Q.
  tmp <- HAnat__erf@OR
  tmp[!is.finite(tmp)] <- NA 
  HAnat__randomF_rf_evaluation$Q <- (mean(tmp,na.rm=T)-1)/(mean(tmp,na.rm=T)+1)
  HAnat__randomF_rf_evaluation$TSS <- mean(HAnat__erf@TPR,na.rm=T)+mean(HAnat__erf@TNR,na.rm=T)-1
  HAnat__randomF_evaluation_total <- rbind(HAnat__randomF_evaluation_total,HAnat__randomF_rf_evaluation)
}

# p <- as.matrix(HAnat_rf_regress$importance)   
# ord <- rev(order(p[,1], decreasing=TRUE)[1:dim(p)[1]]) 
# dotchart(p[ord,1], main="HAnat Variable Importance")  
# 
#Summarie Evaluations and save as an aggregated table
tmpMean <- colMeans(HAnat__randomF_evaluation_total)
tmpSD <- apply(HAnat__randomF_evaluation_total,2,sd)
HAnat__randomF_evaluation_total_summarized <- as.data.frame(rbind(tmpMean,tmpSD))
#To save aggregated data frame.
write.table(HAnat__randomF_evaluation_total_summarized,paste("HAnatRFEvaluations.txt",sep=""),quote=FALSE,sep="\t",row.names = TRUE)
...
#####Current Predictions#####
# HAnat_with_envi <- read.csv("HAnat_with_envi.csv")
...
png(paste("HAnatPartialDependence.png",sep=""),width=2*800,height=400*length(envi_filename))
