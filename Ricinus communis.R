...
Ricinuscommunis <- "Ricinus communis"
RCinvObservations <- input[input$verbatimScientificName %in% Ricinuscommunis, ]

write.csv(RCinvObservations, file="RCinvObservations")
...
RCinv_location <- RCinvObservations[, c("decimalLongitude", "decimalLatitude")]
...
RCinv_with_envi <- RCinv_location
for(envi_map_layer in envi_map_layers){
  envi_filename <- gsub("./EnvironmentalFactors/","",gsub(".tif","",envi_map_layer))
  # RCinv species
  RCinv <- as.data.frame(raster::extract(raster(envi_map_layer),RCinv_location))
  colnames(RCinv) <- envi_filename
  RCinv_with_envi <- cbind(RCinv_with_envi,RCinv)
}

RCinv_with_envi$pa <- 1

# write.csv(RCinv_with_envi, file="RCinv_with_envi.csv")
# write.csv(native_with_envi, file="native_with_envi.csv")

#Remove rows with missing data for observations points.
RCinv_with_envi <- na.omit(RCinv_with_envi)

# RCinv_with_envi$X <- NULL
...
lat_min <- 33
lat_max <- 35
#Get the larger of the two numbers: native or RCinv species observations.
obs_num <- 10*max(nrow(RCinv_with_envi))

#Create empty dataframe for random background points for RCinvs.
#Make it much larger than the set of presence points.
background_RCinv_locations <- data.frame(matrix(nrow=obs_num,ncol=2))
colnames(background_RCinv_locations) <- c("decimalLongitude","decimalLatitude")
#Randomly generate longitude and latitude points.
background_RCinv_locations$decimalLongitude <- runif(obs_num,min=long_min,max=long_max)
background_RCinv_locations$decimalLatitude <- runif(obs_num,min=lat_min,max=lat_max)
...
RCinv_background <- background_RCinv_locations
for(envi_map_layer in envi_map_layers){
  envi_filename <- gsub("./EnvironmentalFactors/","",gsub(".tif","",envi_map_layer)) #Get short names for environmental layers.
  #Extract environmental values at RCinv species locations.
  RCinv_background_tmp <- as.data.frame(raster::extract(raster(envi_map_layer),background_RCinv_locations))
  colnames(RCinv_background_tmp) <- envi_filename
  RCinv_background <- cbind(RCinv_background,RCinv_background_tmp)
  #Extract environmental values at native species locations.
}

RCinv_background$pa <- 0



#Remove rows with missing data for observations points.
RCinv_background <- na.omit(RCinv_background)


#Run random forest models on your RCinv species data.
...
for(i in 1:model_num){
  pres_subset <- RCinv_with_envi[,colnames(RCinv_with_envi) %in% c(envi_filename,"pa")] #Remove latitude and longitude columns from presence point subset.
  pres_subset <- pres_subset[sample(nrow(pres_subset),sample_num),] #Subsample rows.
  abs_subset <- RCinv_background[,colnames(RCinv_background) %in% c(envi_filename,"pa")] #Remove latitude and longitude columns from presence point subset.
...
write.table(rf_importance_total_sum,"RCinvVarImp.txt",quote=FALSE,sep="\t",row.names = FALSE)
...
redo_RCinv_with_envi <- RCinv_location
for(envi_map_layer in envi_map_layers){
  envi_filename <- gsub("./EnvironmentalFactors/","",gsub(".tif","",envi_map_layer))
  # RCinv species
  RCinv <- as.data.frame(raster::extract(raster(envi_map_layer),RCinv_location))
  colnames(RCinv) <- envi_filename
  redo_RCinv_with_envi <- cbind(redo_RCinv_with_envi,RCinv)
  # native species
}

redo_RCinv_with_envi$pa <- 1
# 
# write.csv(redo_RCinv_with_envi, file="redo_RCinv_with_envi.csv")
# write.csv(redo_native_with_envi, file="redo_native_with_envi.csv")

#Remove rows with missing data for observations points.
redo_RCinv_with_envi <- na.omit(redo_RCinv_with_envi)
...
lat_max <- 35
#Get the larger of the two numbers: native or RCinv species observations.
obs_num <- 10*max(nrow(redo_RCinv_with_envi))

#Create empty dataframe for random background points for RCinvs.
#Make it much larger than the set of presence points.
redo_background_RCinv_locations <- data.frame(matrix(nrow=obs_num,ncol=2))
colnames(redo_background_RCinv_locations) <- c("decimalLongitude","decimalLatitude")
#Randomly generate longitude and latitude points.
redo_background_RCinv_locations$decimalLongitude <- runif(obs_num,min=long_min,max=long_max)
redo_background_RCinv_locations$decimalLatitude <- runif(obs_num,min=lat_min,max=lat_max)
...
redo_RCinv_background <- redo_background_RCinv_locations
for(envi_map_layer in envi_map_layers){
  envi_filename <- gsub("./EnvironmentalFactors/","",gsub(".tif","",envi_map_layer))
  #Get short names for environmental layers.
  #Extract environmental values at RCinv species locations.
  redo_RCinv_background_tmp <- as.data.frame(raster::extract(raster(envi_map_layer),redo_background_RCinv_locations))
  colnames(redo_RCinv_background_tmp) <- envi_filename
  redo_RCinv_background <- cbind(redo_RCinv_background,redo_RCinv_background_tmp)
  #Extract environmental values at native species locations.
}

#Designate a presence/absence column for the pseudo-absence points.
redo_RCinv_background$pa <- 0



#Remove rows with missing data for observations points.
redo_RCinv_background <- na.omit(redo_RCinv_background)




RCinv__randomF_rf_importance_total <- data.frame()
RCinv__randomF_evaluation_total <- data.frame()

envi_filename <- gsub("./EnvironmentalFactors/","",gsub(".tif","",envi_map_layers))
for(i in 1:model_num){
  pres_subset <- redo_RCinv_with_envi[,colnames(redo_RCinv_with_envi) %in% c(envi_filename,"pa")] #Remove latitude and longitude columns from presence point subset.
  pres_subset <- pres_subset[sample(nrow(pres_subset),sample_num),] #Subsample rows.
  abs_subset <- redo_RCinv_background[,colnames(redo_RCinv_background) %in% c(envi_filename,"pa")] #Remove latitude and longitude columns from presence point subset.
  abs_subset <- abs_subset[sample(nrow(abs_subset),10*sample_num),] #Subsample rows.
  ...
  RCinv_rf_regress <- suppressWarnings(randomForest(envi_train[,colnames(envi_train) %in% envi_filename],envi_train$pa,stepFactor=1,plot=FALSE,doBest=TRUE))
  ...
  RCinv__randomF_rf_importance <- importance(RCinv_rf_regress)
  RCinv__randomF_rf_importance <- data.frame(names=row.names(RCinv__randomF_rf_importance),RCinv__randomF_rf_importance)
  RCinv__randomF_rf_importance_total <- rbind(RCinv__randomF_rf_importance_total,RCinv__randomF_rf_importance)
  
  RCinv__erf <- suppressWarnings(evaluate(testpres,testbackgr,RCinv_rf_regress))
  RCinv__randomF_rf_evaluation <-  data.frame(matrix(nrow=1,ncol=5))
  colnames(RCinv__randomF_rf_evaluation) <- c("AUC","cor","kappa","Q","TSS")
  RCinv__randomF_rf_evaluation$AUC <- RCinv__erf@auc
  RCinv__randomF_rf_evaluation$cor <- RCinv__erf@cor
  RCinv__randomF_rf_evaluation$kappa <- max(RCinv__erf@kappa)
  # Calculate Yule's Q.
  tmp <- RCinv__erf@OR
  tmp[!is.finite(tmp)] <- NA 
  RCinv__randomF_rf_evaluation$Q <- (mean(tmp,na.rm=T)-1)/(mean(tmp,na.rm=T)+1)
  RCinv__randomF_rf_evaluation$TSS <- mean(RCinv__erf@TPR,na.rm=T)+mean(RCinv__erf@TNR,na.rm=T)-1
  RCinv__randomF_evaluation_total <- rbind(RCinv__randomF_evaluation_total,RCinv__randomF_rf_evaluation)
}

# p <- as.matrix(RCinv_rf_regress$importance)   
# ord <- rev(order(p[,1], decreasing=TRUE)[1:dim(p)[1]]) 
# dotchart(p[ord,1], main="RCinv Variable Importance")  
# 
#Summarie Evaluations and save as an aggregated table
tmpMean <- colMeans(RCinv__randomF_evaluation_total)
tmpSD <- apply(RCinv__randomF_evaluation_total,2,sd)
RCinv__randomF_evaluation_total_summarized <- as.data.frame(rbind(tmpMean,tmpSD))
#To save aggregated data frame.
write.table(RCinv__randomF_evaluation_total_summarized,paste("RCinvRFEvaluations.txt",sep=""),quote=FALSE,sep="\t",row.names = TRUE)
...
#####Current Predictions#####
# RCinv_with_envi <- read.csv("RCinv_with_envi.csv")
...
png(paste("RCinvPartialDependence.png",sep=""),width=2*800,height=400*length(envi_filename))
