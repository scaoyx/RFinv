...
######DON'T CARE TO MUCH ABOUT NAME MATHCES--THAT MAY BE A PROBLEM WITH YOUR SEARCH METHOD AND NOT THE DATA ITSELF
  ...
  Marrubiumvulgare <- "Marrubium vulgare"
  MVinvObservations <- input[input$verbatimScientificName %in% Marrubiumvulgare, ]
  
  write.csv(MVinvObservations, file="MVinvObservations")
  ...
  MVinv_location <- MVinvObservations[, c("decimalLongitude", "decimalLatitude")]
  ...
  MVinv_with_envi <- MVinv_location
  for(envi_map_layer in envi_map_layers){
    envi_filename <- gsub("./EnvironmentalFactors/","",gsub(".tif","",envi_map_layer))
    # MVinv species
    MVinv <- as.data.frame(raster::extract(raster(envi_map_layer),MVinv_location))
    colnames(MVinv) <- envi_filename
    MVinv_with_envi <- cbind(MVinv_with_envi,MVinv)
  }
  
  MVinv_with_envi$pa <- 1
  
  # write.csv(MVinv_with_envi, file="MVinv_with_envi.csv")
  # write.csv(native_with_envi, file="native_with_envi.csv")
  
  #Remove rows with missing data for observations points.
  MVinv_with_envi <- na.omit(MVinv_with_envi)
  
  # MVinv_with_envi$X <- NULL
  ...
  lat_min <- 33
  lat_max <- 35
  #Get the larger of the two numbers: native or MVinv species observations.
  obs_num <- 10*max(nrow(MVinv_with_envi))
  
  #Create empty dataframe for random background points for MVinvs.
  #Make it much larger than the set of presence points.
  background_MVinv_locations <- data.frame(matrix(nrow=obs_num,ncol=2))
  colnames(background_MVinv_locations) <- c("decimalLongitude","decimalLatitude")
  #Randomly generate longitude and latitude points.
  background_MVinv_locations$decimalLongitude <- runif(obs_num,min=long_min,max=long_max)
  background_MVinv_locations$decimalLatitude <- runif(obs_num,min=lat_min,max=lat_max)
  ...
  MVinv_background <- background_MVinv_locations
  for(envi_map_layer in envi_map_layers){
    envi_filename <- gsub("./EnvironmentalFactors/","",gsub(".tif","",envi_map_layer)) #Get short names for environmental layers.
    #Extract environmental values at MVinv species locations.
    MVinv_background_tmp <- as.data.frame(raster::extract(raster(envi_map_layer),background_MVinv_locations))
    colnames(MVinv_background_tmp) <- envi_filename
    MVinv_background <- cbind(MVinv_background,MVinv_background_tmp)
    #Extract environmental values at native species locations.
  }
  
  MVinv_background$pa <- 0
  
  
  
  #Remove rows with missing data for observations points.
  MVinv_background <- na.omit(MVinv_background)
  
  
  #Run random forest models on your MVinv species data.
  ...
  sample_num <- 50 #Designate how many points to subsample your data on.
  
  
  envi_filename <- gsub("./EnvironmentalFactors/","",gsub(".tif","",envi_map_layers))
  #This is the version with rf.modelSel
  for(i in 1:model_num){
    pres_subset <- MVinv_with_envi[,colnames(MVinv_with_envi) %in% c(envi_filename,"pa")] #Remove latitude and longitude columns from presence point subset.
    pres_subset <- pres_subset[sample(nrow(pres_subset),sample_num),] #Subsample rows.
    abs_subset <- MVinv_background[,colnames(MVinv_background) %in% c(envi_filename,"pa")] #Remove latitude and longitude columns from presence point subset.
  ...
  write.table(rf_importance_total_sum,"MVinvVarImp.txt",quote=FALSE,sep="\t",row.names = FALSE)
  ...
  redo_MVinv_with_envi <- MVinv_location
  for(envi_map_layer in envi_map_layers){
    envi_filename <- gsub("./EnvironmentalFactors/","",gsub(".tif","",envi_map_layer))
    # MVinv species
    MVinv <- as.data.frame(raster::extract(raster(envi_map_layer),MVinv_location))
    colnames(MVinv) <- envi_filename
    redo_MVinv_with_envi <- cbind(redo_MVinv_with_envi,MVinv)
    # native species
  }
  
  redo_MVinv_with_envi$pa <- 1
  # 
  # write.csv(redo_MVinv_with_envi, file="redo_MVinv_with_envi.csv")
  # write.csv(redo_native_with_envi, file="redo_native_with_envi.csv")
  
  #Remove rows with missing data for observations points.
  redo_MVinv_with_envi <- na.omit(redo_MVinv_with_envi)
  ...
  lat_max <- 35
  #Get the larger of the two numbers: native or MVinv species observations.
  obs_num <- 10*max(nrow(redo_MVinv_with_envi))
  
  #Create empty dataframe for random background points for MVinvs.
  #Make it much larger than the set of presence points.
  redo_background_MVinv_locations <- data.frame(matrix(nrow=obs_num,ncol=2))
  colnames(redo_background_MVinv_locations) <- c("decimalLongitude","decimalLatitude")
  #Randomly generate longitude and latitude points.
  redo_background_MVinv_locations$decimalLongitude <- runif(obs_num,min=long_min,max=long_max)
  redo_background_MVinv_locations$decimalLatitude <- runif(obs_num,min=lat_min,max=lat_max)
  ...
  redo_MVinv_background <- redo_background_MVinv_locations
  for(envi_map_layer in envi_map_layers){
    envi_filename <- gsub("./EnvironmentalFactors/","",gsub(".tif","",envi_map_layer))
    #Get short names for environmental layers.
    #Extract environmental values at MVinv species locations.
    redo_MVinv_background_tmp <- as.data.frame(raster::extract(raster(envi_map_layer),redo_background_MVinv_locations))
    colnames(redo_MVinv_background_tmp) <- envi_filename
    redo_MVinv_background <- cbind(redo_MVinv_background,redo_MVinv_background_tmp)
    #Extract environmental values at native species locations.
  }
  
  #Designate a presence/absence column for the pseudo-absence points.
  redo_MVinv_background$pa <- 0
  
  
  
  #Remove rows with missing data for observations points.
  redo_MVinv_background <- na.omit(redo_MVinv_background)
  
  
  
  
  MVinv__randomF_rf_importance_total <- data.frame()
  MVinv__randomF_evaluation_total <- data.frame()
  
  envi_filename <- gsub("./EnvironmentalFactors/","",gsub(".tif","",envi_map_layers))
  for(i in 1:model_num){
    pres_subset <- redo_MVinv_with_envi[,colnames(redo_MVinv_with_envi) %in% c(envi_filename,"pa")] #Remove latitude and longitude columns from presence point subset.
    pres_subset <- pres_subset[sample(nrow(pres_subset),sample_num),] #Subsample rows.
    abs_subset <- redo_MVinv_background[,colnames(redo_MVinv_background) %in% c(envi_filename,"pa")] #Remove latitude and longitude columns from presence point subset.
    abs_subset <- abs_subset[sample(nrow(abs_subset),10*sample_num),] #Subsample rows.
    ...
    MVinv_rf_regress <- suppressWarnings(randomForest(envi_train[,colnames(envi_train) %in% envi_filename],envi_train$pa,stepFactor=1,plot=FALSE,doBest=TRUE))
    ...
    MVinv__randomF_rf_importance <- importance(MVinv_rf_regress)
    MVinv__randomF_rf_importance <- data.frame(names=row.names(MVinv__randomF_rf_importance),MVinv__randomF_rf_importance)
    MVinv__randomF_rf_importance_total <- rbind(MVinv__randomF_rf_importance_total,MVinv__randomF_rf_importance)
    
    MVinv__erf <- suppressWarnings(evaluate(testpres,testbackgr,MVinv_rf_regress))
    MVinv__randomF_rf_evaluation <-  data.frame(matrix(nrow=1,ncol=5))
    colnames(MVinv__randomF_rf_evaluation) <- c("AUC","cor","kappa","Q","TSS")
    MVinv__randomF_rf_evaluation$AUC <- MVinv__erf@auc
    MVinv__randomF_rf_evaluation$cor <- MVinv__erf@cor
    MVinv__randomF_rf_evaluation$kappa <- max(MVinv__erf@kappa)
    # Calculate Yule's Q.
    tmp <- MVinv__erf@OR
    tmp[!is.finite(tmp)] <- NA 
    MVinv__randomF_rf_evaluation$Q <- (mean(tmp,na.rm=T)-1)/(mean(tmp,na.rm=T)+1)
    MVinv__randomF_rf_evaluation$TSS <- mean(MVinv__erf@TPR,na.rm=T)+mean(MVinv__erf@TNR,na.rm=T)-1
    MVinv__randomF_evaluation_total <- rbind(MVinv__randomF_evaluation_total,MVinv__randomF_rf_evaluation)
  }
  
  # p <- as.matrix(MVinv_rf_regress$importance)   
  # ord <- rev(order(p[,1], decreasing=TRUE)[1:dim(p)[1]]) 
  # dotchart(p[ord,1], main="MVinv Variable Importance")  
  # 
  #Summarie Evaluations and save as an aggregated table
  tmpMean <- colMeans(MVinv__randomF_evaluation_total)
  tmpSD <- apply(MVinv__randomF_evaluation_total,2,sd)
  MVinv__randomF_evaluation_total_summarized <- as.data.frame(rbind(tmpMean,tmpSD))
  #To save aggregated data frame.
  write.table(MVinv__randomF_evaluation_total_summarized,paste("MVinvRFEvaluations.txt",sep=""),quote=FALSE,sep="\t",row.names = TRUE)
  ...
  #####Current Predictions#####
  # MVinv_with_envi <- read.csv("MVinv_with_envi.csv")
  ...
  png(paste("MVinvPartialDependence.png",sep=""),width=2*800,height=400*length(envi_filename))
