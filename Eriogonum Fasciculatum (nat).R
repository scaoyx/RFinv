...
#2. Ricinus communis (EFnat)
#3. Marrubium vulgar (EFnat)

#Native:
#1. Malosma laurina Engl.(EFnat)
#2. Heteromeles arbutifolia (EFnat)
...
Eriogonumfasciculatum <- "Eriogonum fasciculatum"
EFnatObservations <- input[input$verbatimScientificName %in% Eriogonumfasciculatum, ]

write.csv(EFnatObservations, file="EFnatObservations")
...
EFnat_location <- EFnatObservations[, c("decimalLongitude", "decimalLatitude")]
...
EFnat_with_envi <- EFnat_location
for(envi_map_layer in envi_map_layers){
  envi_filename <- gsub("./EnvironmentalFactors/","",gsub(".tif","",envi_map_layer))
  # EFnat species
  EFnat <- as.data.frame(raster::extract(raster(envi_map_layer),EFnat_location))
  colnames(EFnat) <- envi_filename
  EFnat_with_envi <- cbind(EFnat_with_envi,EFnat)
}

EFnat_with_envi$pa <- 1

# write.csv(EFnat_with_envi, file="EFnat_with_envi.csv")
# write.csv(native_with_envi, file="native_with_envi.csv")

#Remove rows with missing data for observations points.
EFnat_with_envi <- na.omit(EFnat_with_envi)

# EFnat_with_envi$X <- NULL
...
lat_min <- 33
lat_max <- 35
#Get the larger of the two numbers: native or EFnat species observations.
obs_num <- 10*max(nrow(EFnat_with_envi))

#Create empty dataframe for random background points for EFnats.
#Make it much larger than the set of presence points.
background_EFnat_locations <- data.frame(matrix(nrow=obs_num,ncol=2))
colnames(background_EFnat_locations) <- c("decimalLongitude","decimalLatitude")
#Randomly generate longitude and latitude points.
background_EFnat_locations$decimalLongitude <- runif(obs_num,min=long_min,max=long_max)
background_EFnat_locations$decimalLatitude <- runif(obs_num,min=lat_min,max=lat_max)
...
EFnat_background <- background_EFnat_locations
for(envi_map_layer in envi_map_layers){
  envi_filename <- gsub("./EnvironmentalFactors/","",gsub(".tif","",envi_map_layer)) #Get short names for environmental layers.
  #Extract environmental values at EFnat species locations.
  EFnat_background_tmp <- as.data.frame(raster::extract(raster(envi_map_layer),background_EFnat_locations))
  colnames(EFnat_background_tmp) <- envi_filename
  EFnat_background <- cbind(EFnat_background,EFnat_background_tmp)
  #Extract environmental values at native species locations.
}

EFnat_background$pa <- 0



#Remove rows with missing data for observations points.
EFnat_background <- na.omit(EFnat_background)


#Run random forest models on your EFnat species data.
...
sample_num <- 90 #Designate how many points to subsample your data on.


envi_filename <- gsub("./EnvironmentalFactors/","",gsub(".tif","",envi_map_layers))
#This is the version with rf.modelSel
for(i in 1:model_num){
  pres_subset <- EFnat_with_envi[,colnames(EFnat_with_envi) %in% c(envi_filename,"pa")] #Remove latitude and longitude columns from presence point subset.
  pres_subset <- pres_subset[sample(nrow(pres_subset),sample_num),] #Subsample rows.
  abs_subset <- EFnat_background[,colnames(EFnat_background) %in% c(envi_filename,"pa")] #Remove latitude and longitude columns from presence point subset.
...
write.table(rf_importance_total_sum,"EFnatVarImp.txt",quote=FALSE,sep="\t",row.names = FALSE)
...
redo_EFnat_with_envi <- EFnat_location
for(envi_map_layer in envi_map_layers){
  envi_filename <- gsub("./EnvironmentalFactors/","",gsub(".tif","",envi_map_layer))
  # EFnat species
  EFnat <- as.data.frame(raster::extract(raster(envi_map_layer),EFnat_location))
  colnames(EFnat) <- envi_filename
  redo_EFnat_with_envi <- cbind(redo_EFnat_with_envi,EFnat)
  # native species
}

redo_EFnat_with_envi$pa <- 1
# 
# write.csv(redo_EFnat_with_envi, file="redo_EFnat_with_envi.csv")
# write.csv(redo_native_with_envi, file="redo_native_with_envi.csv")

#Remove rows with missing data for observations points.
redo_EFnat_with_envi <- na.omit(redo_EFnat_with_envi)
...
lat_max <- 35
#Get the larger of the two numbers: native or EFnat species observations.
obs_num <- 10*max(nrow(redo_EFnat_with_envi))

#Create empty dataframe for random background points for EFnats.
#Make it much larger than the set of presence points.
redo_background_EFnat_locations <- data.frame(matrix(nrow=obs_num,ncol=2))
colnames(redo_background_EFnat_locations) <- c("decimalLongitude","decimalLatitude")
#Randomly generate longitude and latitude points.
redo_background_EFnat_locations$decimalLongitude <- runif(obs_num,min=long_min,max=long_max)
redo_background_EFnat_locations$decimalLatitude <- runif(obs_num,min=lat_min,max=lat_max)
...
redo_EFnat_background <- redo_background_EFnat_locations
for(envi_map_layer in envi_map_layers){
  envi_filename <- gsub("./EnvironmentalFactors/","",gsub(".tif","",envi_map_layer))
  #Get short names for environmental layers.
  #Extract environmental values at EFnat species locations.
  redo_EFnat_background_tmp <- as.data.frame(raster::extract(raster(envi_map_layer),redo_background_EFnat_locations))
  colnames(redo_EFnat_background_tmp) <- envi_filename
  redo_EFnat_background <- cbind(redo_EFnat_background,redo_EFnat_background_tmp)
  #Extract environmental values at native species locations.
}

#Designate a presence/absence column for the pseudo-absence points.
redo_EFnat_background$pa <- 0



#Remove rows with missing data for observations points.
redo_EFnat_background <- na.omit(redo_EFnat_background)




EFnat__randomF_rf_importance_total <- data.frame()
EFnat__randomF_evaluation_total <- data.frame()

envi_filename <- gsub("./EnvironmentalFactors/","",gsub(".tif","",envi_map_layers))
for(i in 1:model_num){
  pres_subset <- redo_EFnat_with_envi[,colnames(redo_EFnat_with_envi) %in% c(envi_filename,"pa")] #Remove latitude and longitude columns from presence point subset.
  pres_subset <- pres_subset[sample(nrow(pres_subset),sample_num),] #Subsample rows.
  abs_subset <- redo_EFnat_background[,colnames(redo_EFnat_background) %in% c(envi_filename,"pa")] #Remove latitude and longitude columns from presence point subset.
  abs_subset <- abs_subset[sample(nrow(abs_subset),10*sample_num),] #Subsample rows.
  ...
  EFnat_rf_regress <- suppressWarnings(randomForest(envi_train[,colnames(envi_train) %in% envi_filename],envi_train$pa,stepFactor=1,plot=FALSE,doBest=TRUE))
  ...
  EFnat__randomF_rf_importance <- importance(EFnat_rf_regress)
  EFnat__randomF_rf_importance <- data.frame(names=row.names(EFnat__randomF_rf_importance),EFnat__randomF_rf_importance)
  EFnat__randomF_rf_importance_total <- rbind(EFnat__randomF_rf_importance_total,EFnat__randomF_rf_importance)
  
  EFnat__erf <- suppressWarnings(evaluate(testpres,testbackgr,EFnat_rf_regress))
  EFnat__randomF_rf_evaluation <-  data.frame(matrix(nrow=1,ncol=5))
  colnames(EFnat__randomF_rf_evaluation) <- c("AUC","cor","kappa","Q","TSS")
  EFnat__randomF_rf_evaluation$AUC <- EFnat__erf@auc
  EFnat__randomF_rf_evaluation$cor <- EFnat__erf@cor
  EFnat__randomF_rf_evaluation$kappa <- max(EFnat__erf@kappa)
  # Calculate Yule's Q.
  tmp <- EFnat__erf@OR
  tmp[!is.finite(tmp)] <- NA 
  EFnat__randomF_rf_evaluation$Q <- (mean(tmp,na.rm=T)-1)/(mean(tmp,na.rm=T)+1)
  EFnat__randomF_rf_evaluation$TSS <- mean(EFnat__erf@TPR,na.rm=T)+mean(EFnat__erf@TNR,na.rm=T)-1
  EFnat__randomF_evaluation_total <- rbind(EFnat__randomF_evaluation_total,EFnat__randomF_rf_evaluation)
}

# p <- as.matrix(EFnat_rf_regress$importance)   
# ord <- rev(order(p[,1], decreasing=TRUE)[1:dim(p)[1]]) 
# dotchart(p[ord,1], main="EFnat Variable Importance")  
# 
#Summarie Evaluations and save as an aggregated table
tmpMean <- colMeans(EFnat__randomF_evaluation_total)
tmpSD <- apply(EFnat__randomF_evaluation_total,2,sd)
EFnat__randomF_evaluation_total_summarized <- as.data.frame(rbind(tmpMean,tmpSD))
#To save aggregated data frame.
write.table(EFnat__randomF_evaluation_total_summarized,paste("EFnatRFEvaluations.txt",sep=""),quote=FALSE,sep="\t",row.names = TRUE)
...
#####Current Predictions#####
# EFnat_with_envi <- read.csv("EFnat_with_envi.csv")
...
png(paste("EFnatPartialDependence.png",sep=""),width=2*800,height=400*length(envi_filename))
